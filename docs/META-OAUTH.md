...
`pg_cron` job ชื่อ `ads-sync-tick` เรียก `ads_ops.cron_tick()` **วันละครั้งช่วงเช้า 05:00–05:50 เวลาไทย ทุก 10 นาที** (`0,10,20,30,40,50 22 * * *` UTC · ตั้งแต่ 23 ก.ย. 2569 เดิมทุกชั่วโมง) → `net.http_post` ไปที่ Edge Function `ads-cron` (service role เท่านั้น) แล้ว `ads-cron` เป็นคนตัดสินว่ารอบนั้นต้องทำอะไร

ทุกแหล่งถูกดึง **วันละครั้งตามวันที่ไทย** — รอบ 05:00 ทำงานหลัก รอบ 05:10–05:50 เก็บตกเฉพาะงานที่ไม่ทันเพดานเวลา Edge Function (400 วิ) · รอบที่ไม่มีงานเหลืออ่านแค่ฐานข้อมูลเรา ไม่ยิงออกไปหา Meta / ระบบขาย · ตารางเวลาอยู่ที่เดียวใน `supabase/functions/_shared/dailySchedule.js`

| เรื่อง | กติกา |
|---|---|
| ดึงซ้ำ | บัญชีที่วันนี้ (เวลาไทย) ยังไม่มี run ดึงสำเร็จ → ดึง 3 วันล่าสุด · ค่า "ดึงทุก X ชม." เดิมเลิกใช้แล้ว |
...
| ตรวจยอด | วันละครั้งต่อบัญชี ในรอบเช้าหลังดึงเสร็จ และเฉพาะบัญชีที่ไม่มีวันที่ขาด → `ads-reconcile` · ไม่ผ่าน = ตรวจใหม่พรุ่งนี้ |
| กันชนกัน | บัญชีที่มี run กำลังวิ่ง (ยังไม่เกิน 8 นาที) ถูกข้าม · token หมดอายุ = หยุดรอบนั้นทันที |
| ประวัติ | ทุกรอบบันทึกลง `ad_cron_ticks` (รวมรอบที่ไม่มีอะไรต้องทำ) เก็บ 90 วัน · ดูได้ที่หน้าสถานะ Sync แท็บ "ประวัติ" (รอบที่ไม่มีงานไม่แสดง) |
| ยอดขาย | วันละครั้งตี 5 (เวลาไทย) เรียก `sales-sync` ย้อนต้นเดือน (อย่างน้อย 14 วัน) + สำรวจแหล่งของระบบขาย · ลองได้ไม่เกิน 2 ครั้งต่อวันเมื่อไม่สำเร็จ (ไม่หนักเครื่องระบบขาย) |
| Creative | วันละครั้งต่อบัญชี (บัญชีที่รีเฟรชนานสุดก่อน ครั้งละ 1 บัญชีต่อรอบ · 6 รอบเช้า = ได้ถึง 6 บัญชี/วัน) → `ads-creatives` |
| Snapshot บัญชี | วันละครั้ง (เดิมทุกชั่วโมง) — ปุ่ม "ดึง Snapshot บัญชีแอด" ในหน้า Sync ยังสั่งเก็บเดี๋ยวนี้ได้ |
...
หยุดชั่วคราว: `select cron.unschedule('ads-sync-tick');` · เปิดใหม่: `select cron.schedule('ads-sync-tick', '0,10,20,30,40,50 22 * * *', $$select ads_ops.cron_tick();$$);` (migration `20260923090000_ads_cron_daily.sql`)

## 8. วันหมดอายุของ token (ตั้งแต่ 17 ก.ย. 2569)

ตอนแลก token อายุยาว Meta ไม่ได้ส่ง `expires_in` มาทุกครั้ง → `ad_provider_authorizations.expires_at` เคยเป็น null และระบบเตือนล่วงหน้าไม่ได้

ตอนนี้อ่านวันหมดอายุจริงจาก `GET /debug_token` (logic ใน `_shared/metaTokenDebug.js` มีเทส)

| เมื่อไร | ทำอะไร |
|---|---|
| เชื่อม Meta เสร็จ (`ads-oauth-callback`) | ถาม `debug_token` ก่อน · ถามไม่ได้ค่อยใช้ `expires_in` |
| ดึงข้อมูล (`ads-sync`) | ถามวันละครั้งต่อ token หรือทันทีถ้ายังไม่รู้วันหมดอายุ · อัปเดต `expires_at` + `last_verified_at` |
| Meta ตอบว่าใช้ไม่ได้ (`is_valid: false`) | ตั้ง `status = expired` และหยุดรอบนั้นด้วย `META_TOKEN_INVALID` |
| ถามไม่สำเร็จ | ไม่เขียนทับค่าเดิม ดึงข้อมูลต่อตามปกติ |

วันหมดอายุที่เก็บ = ค่าที่มาก่อนระหว่าง `expires_at` (0 = ไม่หมด) กับ `data_access_expires_at` · App token (`App ID|App Secret`) ส่งทาง header ไม่อยู่ใน URL และไม่ log token/URL · `ads-cron` ยังไม่แตะ token เอง ใช้ค่านี้เตือนล่วงหน้า 7 วัน
...
ดึง/ตรวจยอดอัตโนมัติ (ข้อ 7): pg_cron วันละครั้งตี 5 (+ รอบเก็บตกถึง 05:50) + ประวัติทุกรอบใน `ad_cron_ticks`
...
ยอดขายจริงจากระบบขาย (v0.10): `sales-sync` ดูสเปกที่ `docs/superpowers/specs/2026-09-16-sales-revenue-bridge.md`
ขั้นถัดไป: ยอดขาย JUNTAKARN (Supabase ของอาร์ต) · แจ้งเตือนเมื่อตรวจยอดไม่ผ่านติดกันหลายวัน
