...
> **หมายเหตุสำคัญ:** SOP board 7 ขั้น (บอร์ด · ลิสต์ · ปฏิทิน · คิวรอตรวจ · คลัง · Dashboard 4 แท็บ) ถูกถอดออกจากเส้นทางแอปตั้งแต่ commit `0fc984f refactor(marketing): keep only ads workspace` แผนเดิมที่ว่าด้วยการรื้อผิวเหล่านั้นจึงยกเลิก ถ้าวันหนึ่งจะรื้อฟื้น ให้ถือเป็นโปรเจกต์ใหม่และเขียนแผนใหม่
...
| # | ผิว | ไฟล์หลัก | สถานะ |
|---|---|---|---|
| 2.1 | ตั้งค่า ads | `ads/AdsControlCenter.jsx` | ⬜ ยังไม่ audit (หน้าที่ใหญ่สุด คนใช้ทุกครั้งที่ตั้งบัญชี/เป้า) |
| 2.2 | แคมเปญ + drawer | `campaigns/*` | 🔧 drawer รื้อแล้ว 16 ก.ย. · ตัวตารางยังไม่ audit |
| 2.3 | Creative | `creatives/*` | ✅ รื้อแล้ว (สื่อ 1:1 · เลื่อนภาพ · pagination) ยังไม่ผ่าน audit เต็มรูป |
| 2.4 | Overview ads | `ads/AdsWorkspace.jsx` | ⬜ ยังไม่ audit |
| 2.5 | สถานะ Sync | `ads/SyncStatusView.jsx` | 🔧 เพิ่มกล่องดึงอัตโนมัติแล้ว · ตัวอักษรส่วนเก่ายังเล็ก (8–9px) |

### ข้อ 3 — หนี้ที่รู้ตัว

- `mktStyles.css` 3,606 บรรทัด มีกฎของหน้าที่ถอดออกไปแล้วปนอยู่มาก — ต้องไล่ลบแบบยืนยันทีละคลาส (เคยพังมา 3 ครั้งจากสคริปต์ลบอัตโนมัติ)
- โหมดเดโมมี error ค้าง `data/supabaseStore.js` เรียก `.rpc` ทั้งที่ client เป็น null
