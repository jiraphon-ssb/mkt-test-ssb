import { corsHeaders, decryptToken, graph, json, requireTeamLead } from "../_shared/adsOAuth.ts";
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (revoke) {
      ...
      catch (error) { revokeWarning = error instanceof Error ? error.message : "META_REVOKE_FAILED"; }
    }
