...
function CreativeCard({ row, checked, onToggle, onPreview, ruleResult }) {
  ...
  return <article className={`cl-card ${row.fatigue ? "is-fatigue" : ""}`}>
    ...
    <div className="cl-card-body">
      ...
      <div className="cl-metrics"><div><span>ค่าแอด</span><b>{metric(row.spend, "money")}</b></div><div><span>การซื้อ</span><b title={row.purchases == null ? "บัญชีนี้ Meta ไม่ได้วัดการซื้อ" : undefined}>{metric(row.purchases, "count")}</b></div><div><span>ต่อการซื้อ</span><b>{metric(row.cpa, "money")}</b></div><div><span>ROAS การซื้อ</span><b>{metric(row.roas, "roas")}</b></div><div><span>ต่อการเริ่มสนทนา</span><b>{metric(row.cpl, "money")}</b></div><div><span>CTR (ทั้งหมด)</span><b>{metric(row.ctr, "pct")}</b></div>
...
function CompareTray({ rows, onRemove, onClear }) {
  ...
  return <section className="cl-compare" aria-label="เปรียบเทียบครีเอทีฟ"><header><div><strong>เทียบ {rows.length} ชิ้น</strong><span>ดูบนฐานช่วงเวลาเดียวกัน</span></div><button type="button" onClick={onClear}>ล้างทั้งหมด</button></header><div className="cl-compare-grid">{rows.map((row) => <article key={row.key}><button type="button" aria-label={`เอา ${row.creative} ออกจากการเปรียบเทียบ`} onClick={() => onRemove(row.key)}><X size={14} /></button><strong>{row.creative}</strong><small>{row.brand} · {row.platform}</small><dl><div><dt>ค่าแอด</dt><dd>{metric(row.spend, "money")}</dd></div><div><dt>การซื้อ</dt><dd>{metric(row.purchases, "count")}</dd></div><div><dt>ต่อการซื้อ</dt><dd>{metric(row.cpa, "money")}</dd></div><div><dt>ROAS การซื้อ</dt><dd>{metric(row.roas, "roas")}</dd></div><div><dt>ต่อการเริ่มสนทนา</dt><dd>{metric(row.cpl, "money")}</dd></div><div><dt>CTR (ทั้งหมด)</dt><dd>{metric(row.ctr, "pct")}</dd></div><div><dt>%Ads</dt><dd>{metric(row.revenue > 0 ? row.spend / row.revenue : null, "pct")}</dd></div></dl></article>)}</div></section>;
}
...
export function CreativeLibraryView() {
  ...
  return <main className="aw cl">
    ...
    {v.formats.length > 0 && <section className="cl-formats" aria-labelledby="cl-formats-h"><h2 id="cl-formats-h">เทียบตามรูปแบบชิ้นงาน</h2>
      <div className="aw-table-scroll"><table><thead><tr><th>รูปแบบ</th><th>ชิ้น</th><th>ค่าแอด</th><th>สัดส่วน</th><th>ต่อการเริ่มสนทนา</th><th>การซื้อ</th><th>ต่อการซื้อ</th><th>CTR (ทั้งหมด)</th><th>เริ่มล้า</th></tr></thead>
