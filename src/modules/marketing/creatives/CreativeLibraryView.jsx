...
const metric = (value, format = "number", currency = "THB") => value == null ? "—" : format === "money"
  ...
  : format === "pct" ? fmtPct(value, 2) : format === "count" ? fmtInt(value) : `${fmtNum(value, 2)}x`;
...
function CreativeCard({ row, checked, onToggle, onPreview, ruleResult }) {
  ...
  const resultCostLabel = row.resultLabel && row.resultLabel !== "ผลลัพธ์" ? `ต่อ${row.resultLabel}` : "ต่อผลลัพธ์ Meta";
  return <article className={`cl-card ${row.fatigue ? "is-fatigue" : ""}`}>
    ...
    <div className="cl-card-body">
      ...
      <div className="cl-metrics"><div><span>ค่าแอด</span><b>{metric(row.spend, "money", row.currency)}</b></div><div><span>การซื้อ</span><b title={row.purchases == null ? "บัญชีนี้ Meta ไม่ได้วัดการซื้อ" : undefined}>{metric(row.purchases, "count")}</b></div><div><span>ต่อการซื้อ</span><b>{metric(row.cpa, "money", row.currency)}</b></div><div><span>ROAS การซื้อ</span><b>{metric(row.roas, "roas")}</b></div><div><span title={row.resultEvents?.length ? `Meta event: ${row.resultEvents.join(", ")}` : undefined}>{resultCostLabel}</span><b>{metric(row.cpl, "money", row.currency)}</b></div><div><span>CTR (ทั้งหมด)</span><b>{metric(row.ctr, "pct")}</b></div>
        {/* %Ads = ค่าแอด ÷ รายได้ที่ Meta วัดได้ (อาร์ตขอ 21 ก.ย. ค่ำ) — บัญชีที่ Meta ไม่วัดรายได้ = ขีด ไม่ใช่ศูนย์ */}
        <div><span>%Ads</span><b title={row.revenue == null ? "บัญชีนี้ Meta ไม่ได้วัดรายได้" : undefined}>{metric(row.revenue > 0 ? row.spend / row.revenue : null, "pct")}</b></div></div>
...
function CompareTray({ rows, onRemove, onClear }) {
  ...
  return <section className="cl-compare" aria-label="เปรียบเทียบครีเอทีฟ"><header><div><strong>เทียบ {rows.length} ชิ้น</strong><span>ดูบนฐานช่วงเวลาเดียวกัน</span></div><button type="button" onClick={onClear}>ล้างทั้งหมด</button></header><div className="cl-compare-grid">{rows.map((row) => <article key={row.key}><button type="button" aria-label={`เอา ${row.creative} ออกจากการเปรียบเทียบ`} onClick={() => onRemove(row.key)}><X size={14} /></button><strong>{row.creative}</strong><small>{row.brand} · {row.platform}</small><dl><div><dt>ค่าแอด</dt><dd>{metric(row.spend, "money")}</dd></div><div><dt>การซื้อ</dt><dd>{metric(row.purchases, "count")}</dd></div><div><dt>ต่อการซื้อ</dt><dd>{metric(row.cpa, "money")}</dd></div><div><dt>ROAS การซื้อ</dt><dd>{metric(row.roas, "roas")}</dd></div><div><dt>ต่อผลลัพธ์ Meta</dt><dd>{metric(row.cpl, "money")}</dd></div><div><dt>CTR (ทั้งหมด)</dt><dd>{metric(row.ctr, "pct")}</dd></div><div><dt>%Ads</dt><dd>{metric(row.revenue > 0 ? row.spend / row.revenue : null, "pct")}</dd></div><div><dt>ความถี่เฉลี่ย/วัน</dt><dd>{metric(row.frequency)}</dd></div></dl></article>)}</div></section>;
}
...
export function CreativeLibraryView() {
  ...
  return <main className="aw cl">
    ...
    {v.formats.length > 0 && <section className="cl-formats" aria-labelledby="cl-formats-h"><h2 id="cl-formats-h">เทียบตามรูปแบบชิ้นงาน</h2>
      ...
      <p className="aw-key">รูปแบบมาจาก Meta ถ้า Meta ไม่ระบุ อ่านจากคำนำหน้าชื่อชิ้นงาน (VDO · PIC · Album) · ผลลัพธ์คนละ event จะไม่ถูกรวมเป็นต้นทุนเดียว · "ต่ำสุด" เทียบเฉพาะรูปแบบที่มีตั้งแต่ 3 ชิ้น · กดชื่อรูปแบบเพื่อกรอง</p>
    </section>}
