...
import { AlertTriangle, ArrowRight, CheckCircle2, Clock3, Database, Download, RefreshCw, Settings2 } from "lucide-react";
...
import { applyConnectionResult, runSyncQueue } from "./adsConnectionSync.js";
...
export function SyncStatusView() {
  ...
  const config = useMemo(() => dbConnections ? applyConnectionResult(saved ?? {}, { connections: dbConnections }) : (saved ?? {}), [saved, dbConnections]);
  ...
  const refresh = async () => {
    setLoading(true);
    try {
      const [runsFromDb, connections] = await Promise.all([apiClient.ads.recentSyncs(20), canSync ? apiClient.ads.connections() : Promise.resolve(null)]);
